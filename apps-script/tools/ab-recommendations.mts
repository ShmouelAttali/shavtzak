...
// Locate the header the way resolveScheduleLayout_ does, rather than assuming a
// row number: rows get inserted above the task table (the J3:K5 summary block
// pushed it from row 3 to row 6), and hardcoding it here would silently count
// the header as a task row and mislabel every diff.
const clean = (v: unknown) => String(v ?? '').replace(/[״"'׳]/g, '').trim();
const headerIdx = tasks.findIndex((r) => {
  const cells = r.map(clean);
  return cells.includes('העמדה') && cells.includes('החייל');
});
if (headerIdx === -1) throw new Error('no header row in "שבצק" containing both העמדה and החייל');

const header = tasks[headerIdx].map(clean);
const col = {
...
const rowInfo = (sheetRow: number) => {
  ...
  const at = (i: number) => (i === -1 ? '' : String(r[i] ?? ''));
  return { date: at(col.date), position: at(col.position), type: at(col.type), time: at(col.time), soldier: at(col.soldier) };
};

const taskRows = tasks
  .map((_, i) => i + 1)
  .filter((n) => n > headerIdx + 1 && (rowInfo(n).position || rowInfo(n).type));
