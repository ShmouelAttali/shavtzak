...
-- The SQL below is an exact mirror of the JS, generated by NORMALIZE_SQL in
-- api/exit-requests.ts — the two must stay character-identical or the query's
-- LHS stops matching the stored index expression and the index goes unused:
--     regexp_replace [״"׳'`] → ''   (JS .replace(/[״"׳'`]/g, ''))
--     regexp_replace [<JS \s class>]+ → ' '   (JS .replace(/\s+/g, ' '))
--     btrim(…, ' ')                 (JS .trim(); after the collapse only
--                                    plain spaces can be left at the edges)
-- The whitespace class ENUMERATES every codepoint JS \s matches — Postgres \s
-- ([[:space:]]) misses the non-breaking ones (NBSP U+00A0, figure space
-- U+2007, narrow NBSP U+202F, BOM U+FEFF), exactly the characters a
-- Google-Sheets paste produces. Both functions are IMMUTABLE, so index-able.
-- Caller: api/exit-requests.ts resolveSoldier() — its normalized fallback
-- used to read the WHOLE roster and compare in JS. With this index the
